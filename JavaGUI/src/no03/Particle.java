...
public class Particle {
	...
	public Particle(float x, float y) {
		...
		gv = 0.05f;
		state = Particle.STATE_WAITING;
		timer = (int) (Math.random() * 150.0f);
	}
	...
	public void waiting() {
		...
		if (this.timer <= 0) {
			this.state = Particle.STATE_MOVE;
		}
	}
	...
	public void update(JsGameCanvas jgc) {
		if (this.state == Particle.STATE_WAITING) {
			this.waiting();
		} else if (this.state == Particle.STATE_MOVE) {
			this.move();
		}
	}
